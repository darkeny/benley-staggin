...
const Loans: React.FC = () => {
    ...
    const [advanceMethod, setAdvanceMethod] = useState('Transferência');
    ...
    const handleRegisterAdvance = async () => {
        if (!expandedLoanId || !advanceAmount || isSubmittingAdvance) return;

        setIsSubmittingAdvance(true);
        try {
            const response = await fetch(`${apiUrl}/installments/advance`, {
                method: 'POST',
                headers: {
            ...
            setAlertText('Pagamento antecipado registrado com sucesso!');
            ...
            fetchLoans(apiUrl, user, (data: Loan[]) => {
                setLoans(data);
                const updatedLoan = data.find(l => l.id === expandedLoanId);
                if (updatedLoan) {
                    setInstallments(updatedLoan.installmentsList || []);
                }
            });
                                                ...
                                                <tr className="bg-gray-50">
                                                    <td colSpan={visibleColumns.length + 1} className="p-4">
                                                        <div className="bg-white rounded-lg border border-gray-200 p-4">
                                                                     ...
                                                                     <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
                                                                         <div className="p-6">
                                                                             <div className="flex items-center gap-3 mb-6">
                                                                                 ...
                                                                                 <div>
                                                                                     <h3 className="text-lg font-bold text-slate-900">Registrar Adiantamento</h3>
                                                                                     <p className="text-sm text-slate-500">O valor será descontado do saldo devedor.</p>
                                                                                 </div>
                                                                             </div>

                                                                             <div className="space-y-4">
                                                                                 <div>
                                                                                     <label className="block text-sm font-medium text-slate-700 mb-1.5">Valor do Pagamento (MT)</label>
                                                                                     <input
                                                                                         type="number"
                                                                                         value={advanceAmount}
                                                                             ...
                                                                             <div className="flex items-center gap-3 mt-8">
                                                                                 ...
                                                                                 <button
                                                                                     ...
                                                                                     className="flex-1 px-4 py-2.5 text-sm font-bold text-white bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed rounded-xl transition-all shadow-md active:scale-[0.98]"
                                                                                 >
                                                                                     {isSubmittingAdvance ? 'Processando...' : 'Confirmar Pagamento'}
                                                                                 </button>
                                                                             </div>
                                                                         </div>
                                                                     </div>
                                                             ...
                                                             {loan.advancePayments && loan.advancePayments.length > 0 && (
                                                                     ...
                                                                     <div className="overflow-x-auto">
                                                                         <table className="min-w-full divide-y divide-gray-200">
                                                                             ...
                                                                             <tbody className="divide-y divide-gray-200">
                                                                                 {loan.advancePayments.map((adv: AdvancePayment, idx: number) => (
                                                                                     <tr key={adv.id} className="hover:bg-slate-50 transition-colors">
                                                                                         <td className="px-4 py-2 text-sm text-slate-600">
                                                                                             {new Date(adv.paymentDate).toLocaleDateString('pt-BR')}
                                                                                         </td>
